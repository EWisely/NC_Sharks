...
5) 05_lulu_Metabar_MiFish_and_combined_NCSharks.R takes as input MiFish_named_matchlist.txt, MiFish_named_tab_LULU.txt, and the metadata files and the output R environment of 04_lulu_metabaR_BerryCrust_NC_Sharks.R to curate the MiFish MOTUs with their own filtering parameters, and for each species of shark sampled, removes the associated congener MOTUs from downstream diet analyses.  This script then merges the MiFish and Crustacean_16S data back together for each sample and results in a single phyloseq object (NC_Sharks.ps) for downstream analysis.
6) 06_Phyloseq_NC_Sharks.R sorts, normalizes, and does exploratory statistics and visualizations of the data.  This is the stage at which all of the MOTUs representing a single species are aggregated, and differences among sample types and species are explored.  Sørensen distances between fecal swabs and the associated stomach samples of each individual are calculated and visualized as well.  
