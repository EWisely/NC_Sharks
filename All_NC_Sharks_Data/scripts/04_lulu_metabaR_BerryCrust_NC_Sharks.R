...
write.csv(curated_result$curated_table,"All_NC_Sharks_Data/data/05_MetabaR_inputs/lulu_curated_BerryCrust_named.tab")
...
#samples
Metabar_formatted_pcrs<-read.csv("data/05_MetabaR_inputs/Metabar_formatted_pcrs_BerryCrust_lulu.txt", sep="\t")
...
BerryCrust_named_obi3_results<-BerryCrust_named_obi3_results%>% select(ID, COUNT, BEST_IDENTITY, TAXID, SCIENTIFIC_NAME, ID_STATUS,NUC_SEQ)
...
write.csv(Metabar_formatted_reads, "data/05_MetabaR_inputs/Metabar_formatted_reads_pureobi3_lulu.csv")
write.csv(Metabar_formatted_motus, "data/05_MetabaR_inputs/Metabar_formatted_motus_pureobi3_lulu.csv")
write.csv(Metabar_formatted_pcrs, "data/05_MetabaR_inputs/Metabar_formatted_pcrs_pureobi3_lulu.csv")
write.csv(Metabar_formatted_samples, "data/05_MetabaR_inputs/Metabar_formatted_samples_pureobi3_lulu.csv")
...
#  file_reads= "data/05_MetabaR_inputs/Metabar_formatted_reads_pureobi3_lulu.txt",
#  file_motus= "data/05_MetabaR_inputs/Metabar_formatted_motus_pureobi3_lulu.txt",
#  file_pcrs ="data/05_MetabaR_inputs/Metabar_formatted_pcrs_pureobi3_lulu.txt",
...
negative_control_id<- grepl("neg", rownames(NC_BerryCrust$pcrs))
NC_BerryCrust_Negatives <- subset_metabarlist(NC_BerryCrust, table = "pcrs", indices = negative_control_id)  
...
NC_BerryCrust_contams<-subset_metabarlist(NC_BerryCrust, table="motus", indices=id)
summary_metabarlist(NC_BerryCrust_contams)
...
# Compute the number of reads per pcr
NC_BerryCrust1$pcrs$nb_reads <- rowSums(NC_BerryCrust1$reads)
...
#I tried changing the tag-jump filtering to 0.001 and that added more samples back in, and changing the cutoff back to the pipeline 97% didn't do much.
  ...
  #in the end I went with 0.01% cutoff for tagjumps and 98% similarity.
...
write.csv(NC_BerryCrust_clean$reads, "All_NC_Sharks_Data/data/06_MetabaR_results/NC_BerryCrust_obi3_lulu_metabar_reads.csv")
write.csv(NC_BerryCrust_clean$motus, "All_NC_Sharks_Data/data/06_MetabaR_results/NC_BerryCrust_obi3_lulu_metabar_motus.csv")
write.csv(NC_BerryCrust_clean$pcrs, "All_NC_Sharks_Data/data/06_MetabaR_results/NC_BerryCrust_obi3_lulu_metabar_pcrs.csv")
write.csv(NC_BerryCrust_clean$samples, "All_NC_Sharks_Data/data/06_MetabaR_results/NC_BerryCrust_obi3_lulu_metabar_samples.csv")
...
write.csv(df5, "All_NC_Sharks_Data/data/06_MetabaR_results/NC_BerryCrust_clean_species_by_sample_obi3.csv")
